...
def check_datetime(date_time) -> datetime:
    ...
    POSSIBLE_DATE_FORMATS = ["%d.%m.%y %H:%M", "%Y-%m-%d %H:%M:%S.%f"]
    ...
    for date_format in POSSIBLE_DATE_FORMATS:
        try:
            ...
            # if correct format, don't test any other formats
        except ValueError:
    ...
    raise ValueError(f"Timestamp {date_time} format not detected: consider adding it to POSSIBLE_DATE_FORMATS")
...
def get_zscore(value, window) -> float:
    """
    Zscore
    """
    ...
    if len(window) > 0:
        if abs(value - np.average(window)) < 3:
            ...
            zscore = scipy.stats.zscore(np.insert(window, 0, value))[0]
...
def order_insert(date_item, value, month: LoadMonth) -> Union[float, None]:
    ...
    if date_item not in dates:
        ...
        if prev_elem != next_elem:
            ...
            logging.info(
                f"Not ordered sequence of {date_item}:",
                f"value: {value} window: {calc_win} and zscore: {zscore}"
            )
            return zscore
        else:
